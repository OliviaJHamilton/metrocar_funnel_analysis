...
SELECT
  ...
  COUNT(DISTINCT ad.*) AS count_of_users_downloaded_app,
  COUNT(DISTINCT s.user_id) AS total_unique_users_signup,
  COUNT(DISTINCT us.user_id) AS total_users_ride_requested,
  ...
  COUNT(DISTINCT CASE WHEN rr.accept_ts IS NOT NULL THEN rr.ride_id END) AS rides_accepted_by_driver,
  COUNT(DISTINCT CASE WHEN rr.dropoff_ts IS NOT NULL THEN rr.user_id END) AS unique_users_completed_ride,
  COUNT(DISTINCT CASE WHEN rr.dropoff_ts IS NOT NULL THEN rr.ride_id END) AS completed_rides,
...
LEFT JOIN user_funnel_status us ON s.user_id = us.user_id
...
funnel_steps AS (
  SELECT
    ...
    age_range,
    download_date,
    CAST(count_of_users_downloaded_app AS BIGINT) AS user_count,
